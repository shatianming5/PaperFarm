...
Migrated from ``open_researcher.research_events``.  Contains all domain-specific
event dataclasses, the ``ResearchEvent`` type alias, ``EventHandler``, and the
four utility functions (``event_name``, ``event_phase``, ``event_level``,
``event_payload``).
"""
