...
class WorkerManager:
    ...
    def start(self) -> None:
        ...
        n_workers = (
            min(self.max_workers, len(available))
            if self.max_workers > 0 and available
            else self.max_workers if self.max_workers > 0 else 1
        )
    ...
    def _worker_loop(self, worker_id: int, gpu: dict | None) -> None:
        ...
        if gpu:
            ...
            self.gpu_manager.allocate(tag=wid)
            gpu_env = {"CUDA_VISIBLE_DEVICES": str(device)}
            self.on_output(f"[{wid}] Allocated GPU {host}:{device}")
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                if code == 0:
                    self.idea_pool.mark_done(idea["id"], 0.0, "completed")
                else:
        ...
        if gpu:
            try:
                self.gpu_manager.release(gpu["host"], gpu["device"])
            except Exception:
                pass
